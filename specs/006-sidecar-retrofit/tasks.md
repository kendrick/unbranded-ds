...
- [X] T001 Verify spec 005 prerequisites are merged to `main`: confirm `packages/react/src/components/_template/Component.usage.md` exists (sidecar template), `scripts/validate-sidecars.ts` exists and runs clean (compile validator), `AGENTS.md` at repo root contains the component index, and `.github/workflows/ci.yml` contains the "Validate sidecar code blocks" step. Run `pnpm exec tsx scripts/validate-sidecars.ts` once locally to confirm a green baseline.
