...
const SRC_SOURCE = `@source "../src";`;
const DIST_SOURCE = `@source "../dist";`;
...
describe('react preset.css source', () => {
	it('contains exactly the three contracted directives', async () => {
		const contents = await readFile(SRC_PRESET, 'utf8');
		const lines = stripNoise(contents);

		expect(lines).toHaveLength(3);
		expect(lines).toContain(TOKENS_IMPORT);
		expect(lines).toContain(SRC_SOURCE);
		expect(lines).toContain(DIST_SOURCE);
	});
});
